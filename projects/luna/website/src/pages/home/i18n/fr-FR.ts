export default {
  meta: {
    title: 'Découvrez les savoirs ancestraux avec Luna',
    description:
      'Luna connecte les citoyens et professionnels aux pratiques maraîchères ancestrales. Planifiez vos cultures en suivant les cycles lunaires et découvrez des savoirs locaux.',
  },
  hero: {
    title: 'Explorez les savoirs ancestraux pour un maraîchage durable',
    description:
      "Luna vous guide dans l'utilisation des pratiques ancestrales pour vos cultures. Accédez à des outils basés sur les cycles lunaires et découvrez des ressources locales pour préserver la biodiversité et enrichir votre savoir-faire.",
    button: {
      features: {
        label: 'Voir les fonctionnalités',
      },
    },
  },
};
