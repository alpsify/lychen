export default {
  meta: {
    title: 'Reduce Your Waste and Compost with Humu',
    description:
      'Humu helps you manage your biowaste with tools for composting. Connect with your community for sustainable and local practices.',
  },
  hero: {
    title: 'Transform Your Biowaste into Resources with Humu',
    description:
      'Humu makes biowaste management easier through collaborative tools. Participate in local composting initiatives and track the impact of your contributions.',
    button: {
      features: {
        label: 'See Features',
      },
    },
  },
};
