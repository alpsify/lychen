export default {
  meta: {
    title: 'Découvrez et soutenez les magasins de producteurs avec Kolo',
    description:
      'Kolo connecte citoyens et producteurs grâce à une plateforme interactive. Trouvez des magasins locaux, participez à des initiatives solidaires, et gérez vos produits en toute simplicité.',
  },
  hero: {
    title: "Soutenez l'économie locale avec Kolo",
    description:
      "Kolo met en relation citoyens, producteurs et communautés pour renforcer l'économie locale. Localisez les magasins de producteurs, participez à des initiatives solidaires et découvrez des produits du terroir.",
    button: {
      features: {
        label: 'Voir les fonctionnalités',
      },
    },
  },
};
